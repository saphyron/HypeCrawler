...
async function checkForDuplicates() {
  try {
    ...
    // SQL query to find potential duplicates based on title, body, and date (YYYY-MM-DD)
    const query = `
    ...
    if (results.length > 0) {
      ...
      for (const row of results) {
        // Update the possible_duplicate column for each duplicate only if it's not already TRUE
        const updateQuery = `
          UPDATE annonce
    ...
    // Disconnect the database after all operations are complete
