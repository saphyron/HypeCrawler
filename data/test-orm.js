...
async function CreateAnnonceTable() {
...
// SHA-1 selection:
function FindChecksum(incomingChecksum) {
    const query =
        'SELECT * ' +
        'FROM ANNONCE ' +
        'WHERE CHECKSUM = ? ' +
    ...
    connection.query(query, [incomingChecksum], function (error, result, fields) {
 ...
 * @constructor
 */
async function InsertAnnonce(newRecord) {
...
// WIP
async function InsertRecord(tableName, newRecord) {
...
module.exports = {
    CreateAnnonceTable: CreateAnnonceTable,
    InsertAnnonce: InsertAnnonce,
